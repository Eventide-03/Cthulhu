...
//   - the Now Playing player's 16x16 transport icons
//                                          (modules/now-playing/assets/*.png)
//
//   node tools/make-placeholder-art.mjs [--only icons|game|player]
//
...
const PLAYER = path.resolve("src/theme/content/modules/now-playing/assets");
...
/* ------------------- Now Playing transport icons (16x16) ------------------- */
// Rendered at 16 CSS px, pixelated, on the player's dark elevated surface.
const PLAYER_ICONS = {
  prev: [
    "................",
    "................",
...
(async () => {
  ...
  if (!only || only === "player") {
    console.log("player:");
    for (const [name, rows] of Object.entries(PLAYER_ICONS)) {
      await writePng(path.join(PLAYER, name + ".png"), fromMap(rows));
    }
  }
})();
